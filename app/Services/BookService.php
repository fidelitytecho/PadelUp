...
class BookService
{
    ...
    public function __construct(BookingInterface $booking, ServiceInterface $service,
                                PaymobInterface $paymob, PaymentInterface $payment, PlayerInterface $player,
    ...
    public function book(array $data): JsonResponse
    {
        ...
        try {
            ...
            if ($this->isBookingOverlap($data)) {
                ...
                if ($createdBooking) {
                    //check if user has discount and slug == discount_slug
                    $discount = 0;
                    if (auth('api')->user()->promo()->exists() && $data['discount']) {
                        $discount = auth('api')->user()->load('Promo')->Promo->slug == $data['discount'] ? auth('api')->user()->load('Promo')->Promo->discount : 0;
                    ...
                    $userWalletAmount -= $userWalletAmount * $discount;
    ...
    public function createBooking(array $data)
    {
        ...
        $booking = $this->booking->store($data);
        $this->player->create(['booking_id' => $booking->id, 'user_id' => auth('api')->id()]);
        return $booking;
    }
